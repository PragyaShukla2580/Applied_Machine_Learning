...
def prepare(from_date,to_date):
    """
                ...
                ----------
                from_date : a date
                    The date to take the stock price from
                to_date : end date
                    The date till when the stock price is to be extracted
                Description
                ----------
                This function prepares the dataset. It finds the length of the dataset
                and sees if there are any null values to deal with.
                """
...
def preprocess(from_date,to_date):
    """
                    ...
                    Description
                    ----------
                    This function preprocess the dataset. It takes out Close price,
                    scales the dataset in range [0,1], converts data to np.array
                    and finally dumps the dataset using joblib."""
    # Scale the data
    # Get the stock quote
    data = DataReader('GOOG', data_source='yahoo', start=from_date, end=to_date)
...
if __name__=="__main__":
    ...
    try:
        ...
        logging.error("Couldn't complete")
    from_date = data["model_configs_train"]["from_date"]
    to_date = data["model_configs_train"]["to_date"]

    prepare(from_date,to_date)
    preprocess(from_date, to_date)
